...
- **Scene（场）**：已接到编辑器「本章场次」。
- **偏好**：Agent 页可查看、停用。
- **插件按钮**：列出打包清单。「人名点名」可对当前正文运行；其它打包项仍是内置占位。工作流页的「点名当前章」走同一条运行路径。
- **示例章结构**：打开《雾港来客》会预置林默等人，演示「预先设计 → 按段匹配」。`@人物` 标签仍不入库。
- **libraryApi**：设置、续写、偏好、插件、场次、修订历史都走 `libraryApi`（编辑器心跳 / 模式切换除外）。
...
- **上下文三级预算**：钉住/忽略已做。第二级是本机词汇检索（当前段的词去条目标题/说明里找，命中原因 code 为 `retrieve`）。**LLM 重排未做**。
- `apps/client/src/canon/extract.ts` 只给浏览器内存 `proposeCanon` 测试用。
- **界面语言**：设置里可切简体中文 / English。预选条命中原因随界面语言（IPC 仍是 `title:林默` 这种 code）。Rust 错误串、开发者日志、手册正文这一轮仍是中文。写作协议标识不随界面翻译。
